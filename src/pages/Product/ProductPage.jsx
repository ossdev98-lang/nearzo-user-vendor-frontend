...
const ProductPage = () => {
  ...
  const currentOriginalPrice = currentPrice / (1 - (product.discount || 0) / 100)
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pt-20 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-[32px] p-6 sm:p-10 shadow-sm border border-gray-100 dark:border-gray-800">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              >
                ...
                <AnimatePresence mode="wait">
                  <motion.img
                    ...
                    src={activeImage}
                    alt={product.name}
                    className="zoom-target w-full h-full object-contain mix-blend-multiply dark:mix-blend-normal group-hover:scale-[2] transition-transform duration-200 ease-out"
                  />
                </AnimatePresence>
              </div>
            ...
            <div className="flex flex-col">
              <div className="mb-2">
                <button
                  onClick={() => navigate(`/shop/${shop.id}`)}
                  className="inline-block text-primary text-sm font-bold uppercase tracking-wider hover:text-indigo-600 transition-colors underline underline-offset-4 decoration-2"
                >
                  {shop.name}
                </button>
              </div>
              ...
              <div className="flex items-end gap-3 mb-8">
                <span className="text-4xl font-extrabold text-[#6C4CF1]">
              ...
              {/* Variants */}
              <div className="mb-8">
                <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3">Available Options</h3>
                <div className="flex flex-wrap gap-3">
                  {variants.map((v, idx) => (
                    <button
                      key={idx}
                      onClick={() => setSelectedVariant(idx)}
                      className={`px-4 py-2 rounded-xl border-2 font-medium transition-all ${selectedVariant === idx
              ...
              <div className="flex items-center gap-4 mb-10 pb-10 border-b border-gray-100 dark:border-gray-800">
                <div className="flex items-center bg-gray-100 dark:bg-gray-800 rounded-full p-1 border border-gray-200 dark:border-gray-700">
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white hover:text-primary transition-colors"
                  >
                ...
                <button
                  onClick={() => addToCart({ ...product, price: currentPrice, unit: variants[selectedVariant].label })}
                  className={`flex-grow flex items-center justify-center gap-2 px-8 py-4 rounded-full font-bold text-lg transition-all shadow-lg hover:shadow-xl hover:-translate-y-1 ${isInCart
